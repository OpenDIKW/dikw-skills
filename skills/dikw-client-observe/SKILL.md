...
Confirm `dikw-core` is installed and the user has a running server. If setup is
missing, read `../../references/installation.md`; root commands such as
`dikw init`, `dikw serve`, and `dikw auth` are setup context, not this skill's
...
```bash
dikw client info
dikw client status --format json
dikw client health --format json
dikw client check --format json
dikw client check --llm-only --format json
dikw client check --embed-only --format json
```

For humans, use `--format table` where supported.

## Interpretation

- Treat `dikw client health --format json` as the bootstrap probe.
- Treat `dikw client status --format json` as storage/count inspection.
- Treat `dikw client check --format json` as a provider connectivity gate.
